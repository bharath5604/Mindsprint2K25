...
//     // --- MODIFICATION: CORRECTED DUPLICATE CHECK LOGIC ---
//     // 1. Check Team Name (Case-insensitive and space-insensitive)
//     const normalizedTeamName = normalizeString(teamName);
//     if (normalizedTeamName) {
//         // This new aggregation pipeline is compatible with all MongoDB versions.
//         // It splits the name by spaces, then joins the parts back together.
//         const existingTeam = await Participant.aggregate([
//             {
//                 $addFields: {
//                     normalizedDbTeamName: {
//                         $toLower: {
//                             $reduce: {
//                                 input: { $split: ["$teamName", " "] },
//                                 initialValue: "",
...
//     if (normalizedEmail) {
//         // Using the same robust aggregation logic for email
...
//         // Using the same robust logic for member names
//         const existingMember = await Participant.aggregate([
//             { $addFields: { 
//                 normMember1: { $toLower: { $reduce: { input: { $split: [{ $ifNull: ["$member1", ""] }, " "] }, initialValue: "", in: { $concat: ["$$value", "$$this"] } } } },
//                 normMember2: { $toLower: { $reduce: { input: { $split: [{ $ifNull: ["$member2", ""] }, " "] }, initialValue: "", in: { $concat: ["$$value", "$$this"] } } } },
//                 normMember3: { $toLower: { $reduce: { input: { $split: [{ $ifNull: ["$member3", ""] }, " "] }, initialValue: "", in: { $concat: ["$$value", "$$this"] } } } },
//             }},
//             { $match: { $or: [
